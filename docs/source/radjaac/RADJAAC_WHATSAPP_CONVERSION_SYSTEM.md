...
- GA4 Measurement ID dibaca dari environment variable `NEXT_PUBLIC_GA_MEASUREMENT_ID` (tidak di-hardcode), dan analytics hanya aktif pada hostname produksi `www.radjaac.com`.
...
## Radja AC WhatsApp Conversion System
...
- Simpan `wa_source`, `wa_label`, `page_type`, dan `intent_type` sebagai atribut HTML/dataLayer agar gtag dapat mengirimnya sebagai parameter event ke GA4. GA4 mendukung custom event parameters, tetapi agar terbaca di report/exploration, param tersebut harus didaftarkan sebagai custom dimensions.
...
Markup seperti ini harus disesuaikan dengan implementasi repo aktif yang mengirim event melalui gtag langsung.
